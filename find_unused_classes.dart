...
//   _logResults(unusedByType, allClasses.length, stopwatch.elapsedMilliseconds);
...
Future<void> main() async {
  ...
  // خريطة بتربط كل ملف بكل الكلاسات اللي جواه
  final Map<String, List<String>> fileToClasses = {};

  for (var file in libFiles) {
    final content = await file.readAsString();
    final classRegex = RegExp(r'class\s+(\w+)\s+(?:extends|with|implements)?');
    final matches = classRegex.allMatches(content);
    final classes = matches.map((m) => m.group(1)!).toList();
    if (classes.isNotEmpty) {
      fileToClasses[file.path] = classes;
    }
  }

  final Set<String> usedClasses = {};

  // ندور على استخدام كل كلاس في باقي الملفات (lib + test)
  for (var file in allFiles) {
    final content = await file.readAsString();
    for (var classList in fileToClasses.values) {
  ...
  final List<Map<String, dynamic>> unusedReport = [];

  for (var entry in fileToClasses.entries) {
    final filePath = entry.key;
    final unusedInFile =
        entry.value.where((cls) => !usedClasses.contains(cls)).toList();
    if (unusedInFile.isNotEmpty) {
      unusedReport.add({'file': filePath, 'unused_classes': unusedInFile});
    }
  }

  if (unusedReport.isEmpty) {
    log('🎉 كل الكلاسات مستخدمة بفضل الله');
  } else {
    log('🧹 الكلاسات والملفات الغير مستخدمة:\n');
    for (var entry in unusedReport) {
      log('📁 ${entry['file']}');
      for (var cls in entry['unused_classes']) {
        log('   ❌ $cls');
      }
    }

    // لو عايز تحفظها كـ JSON report:
    final jsonReport = JsonEncoder.withIndent('  ').convert(unusedReport);
    final file = File('unused_report.json');
    await file.writeAsString(jsonReport);
    log('\n📄 تم حفظ التقرير في unused_report.json');
  }
}
